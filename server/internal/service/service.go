...
func (api *ApiService) Register(ctx context.Context, body *models.RegisterUserReqBody) (*models.RegisterUserResposne, *models.ErrorResponse) {
	user := &models.User{Email: body.Email, Password: body.Password}
	status, err := api.repo.RegisterUser(ctx, user)
	if err != nil {
		return nil, &models.ErrorResponse{Status: status, Message: err.Error()}
	}
	return &models.RegisterUserResposne{Status: status, Message: "User registered successfully"}, nil
}

func (api *ApiService) GenerateRefreshToken() {

}
func (api *ApiService) DeleteRefreshToken() {
